...
- [ ] T001 [P] Generate the Base UI primitives with the shadcn CLI in `apps/admin` (style `base-nova`, React 18): `pnpm dlx shadcn@latest add dialog alert-dialog switch table` — creates `apps/admin/src/components/ui/dialog.tsx`, `alert-dialog.tsx`, `switch.tsx`, `table.tsx`
- [ ] T002 [P] Create the query key factory in `apps/admin/src/lib/queryKeys.ts` exporting `fareConfigKeys = { all: ["fare-configs"] as const }` (pattern per ADMIN.md §10.1)
