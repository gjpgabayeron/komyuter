...
| ------ | ----------------- | ------------ | ------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------- |
| POST   | `/api/auth/login` | public       | `{ email: string, password: string }` | `{ success: true, data: { access_token: string, user: { id: string, email: string, name: string } } }` | 401 `UNAUTHORIZED` (wrong credentials); 403 `FORBIDDEN` (signed in but not an admin); 422 `VALIDATION_ERROR` (bad body) |
| GET    | `/api/auth/me`    | Bearer token | —                                     | `{ success: true, data: { id: string, email: string, name: string } }`                                 | 401 `UNAUTHORIZED` (missing/invalid/expired token); 403 `FORBIDDEN` (not an admin)                                      |
...
- `name` is the identity's display name, read from `user_metadata.full_name` (`""` when absent); the client falls back to an email-derived display name for the user menu.
