...
| **Community Trust Scoring** *(bonus)* | GPS traces vs. route polylines via Modified Hausdorff Distance → informational trust badges |
...
**Route Management**
- Draw PUJ route polylines directly on map (Mapbox GL + snap-to-road)
- Set route metadata: name, short name, color, direction labels, terminal stops
- Activate/deactivate routes
- Configure detour segments (demand-triggered, direction-dependent)
- Define restricted boarding segments (no-stopping zones)
**Stop Management**
- Place stops along route polylines
- Set stop type: `terminal` | `major_stop` | `waiting_area`
- Toggle `is_guaranteed_service` flag (distinguishes formal stops from hail-and-ride corridor)
- Add landmark hints and AR marker toggle per stop
- Manage stop ordering along route
**Fare Configuration**
...
- Manage route datasets for multiple cities independently
- No vendor dependency — fully admin-controlled
---
...
- Hail-and-ride support — system snaps GPS position to nearest valid polyline point (25–50m tolerance), not just formal stops
- Multi-route disambiguation when multiple jeepney lines share a corridor
**AR Wayfinding** *(activates during walking/transfer segments only)*
...
- **Nodes:** `stop_{stopId}_route_{routeId}` — route-expanded (same physical stop = different node per route)
- **Route edges:** consecutive stops, same route — carry distance + fare
- **Transfer edges:** stops of different routes within 300m — carry walk distance + transfer penalty (1)
- **Detour nodes:** injected temporarily only when passenger triggers detour destination
### Trust Scoring (MHD)
