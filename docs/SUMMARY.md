...
```
Mobile App (Expo) ←→ API (Fastify) ←→ Database (PostgreSQL + PostGIS)
       ↑                    ↑
       │                    │
...
| Graph Construction | Backend            | Build transit graph from routes + directions + stops                |
| Pathfinding        | Backend            | Multi-criteria Dijkstra's algorithm                                 |
| Fare Computation   | Shared Package     | LTFRB distance-based fare formula (exact per-leg fare for display)  |
...
| Supabase Auth                           | All authentication (admin + optional commuter sign-in) |
| `@fastify/rate-limit` + `@fastify/cors` | Rate limiting + CORS                                   |
...
| Supabase Auth    | User auth (admin gating + optional commuter sign-in) |
...
| pnpm 8.15.6                          | Package manager with workspace support (per root `package.json`) |
...
| Fastify Swagger (`@fastify/swagger`) | Interactive API documentation at `/api/docs`                     |
...
| Render (free tier)         | Fastify server (public HTTPS URL baked into the APK) | Free                   |
| Supabase Cloud (free tier) | PostgreSQL + PostGIS + Auth                          | Free                   |
| LAN dev server             | Fastify (local fallback)                             | Free                   |
...
- Maximum speed: 10–45 km/h (max-speed discriminator — rejects static recordings and private cars; congestion-immune, ADR-0003)
...
| 4   | Modified Hausdorff Distance (MHD) | Spatial similarity between trace and route direction polyline | Trace + direction polyline | Average distance in meters |
...
| 8   | Min-Max Normalization     | Scale weights to [0, 1] per edge-type pool (ADR-0002) | Raw weight values               | Normalized weights (clamped) |
| 9   | Multi-Criteria Dijkstra's | Find optimal multi-modal path                         | Graph + origin + dest + profile | Optimal path + directions    |

**Graph Model:** G = (V, E, W)

- **Stop Nodes:** Each stop on each route direction (`stop_{stopId}_direction_{directionId}`)
- **Route Edges:** Between consecutive stops on the same direction (carry distance, marginal fare)
- **Transfer Edges:** Between stops of different routes within 300m (carry walking distance, transfer penalty = 1)
- **Board Edges / Virtual Nodes:** Request-time nodes for non-stop boarding/alighting on the polyline (hail-and-ride), connected to adjacent stops on the same direction
- Fare on route edges = one-time base fare at boarding + marginal ₱1.80/km (ADR-0001); the displayed fare is always recomputed exactly per leg
...
| ------------- | -------------------------------------------------------- | ----------------------------------- |
| users         | User accounts (admin, commuter) — via Supabase Auth      | None                                |
| fare_configs  | LTFRB fare structures                                    | None                                |
| routes        | PUJ route definitions                                    | None (geometry lives on directions) |
| directions    | Directed services of a route (own polyline + stop order) | polyline (LINESTRING, 4326)         |
| stops         | Transit stops along directions                           | location (POINT, 4326)              |
...
> There are **no `graph_nodes`/`graph_edges` tables** — the graph is derived in-memory from routes/directions/stops/fare configs and rebuilt eagerly on mutation (ADR-0005).
...
| Inc 2: Dijkstra's Pathfinding  | Weeks 7–10 (cross-semester) | Obj 2           | Multi-criteria pathfinding with preference profiles; **acceptance criterion:** ≥1 seed-route OD pair where "Cheapest" ≠ "Shortest" output (see T3) |
...
| T3: Switch to "Cheapest" profile     | Obj 2     | 2 min      | Different route/fare noted (OD pair deliberately chosen so the profiles diverge — see Inc 2 acceptance criterion) |
...
| Backend Framework        | Fastify                                | Most documented fast framework with native Zod schema validation and first-class plugin ecosystem; consistent with primary stack docs (ADR-0004) |
